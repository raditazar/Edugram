...
const CreatorPage: React.FC<CreatorPageProps> = ({ onBack }) => {
  ...
  const handleGenerate = async (prompt: string) => {
    try {
      console.log('di klik')
      let imageResponse;
      const narrativeResponse = await generateTextWithGemini({ prompt });
      ...
      localStorage.setItem("imageUrl", imageResponse[0]);
      localStorage.setItem("narrative", narrativeResponse);
        ...
        <div className="w-xl bg-white p-6 rounded-lg relative">
          ...
          <div className="absolute bottom-6 right-6">
            <button
              ...
              onClick={() =>
                handleGenerate(`Develop a story with only just one paragraph with this story's description: ${description}`)
              }
            >
              Generate Content
            </button>
